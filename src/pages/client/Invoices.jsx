  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {invoices.map(inv => (
          <div key={inv.id} style={{ background: 'white', borderRadius: 12, padding: isMobile ? '14px 16px' : '18px 20px', border: '1px solid rgba(0,0,0,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 10 : 0 }}>
            <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, textAlign: isMobile ? 'left' : 'right' }}>
              <div style={{ fontWeight: 700, fontSize: 16, color: '#18181a' }}>${Number(inv.amount).toLocaleString()}</div>
