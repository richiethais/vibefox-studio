...
export default function AdminAccessLinks() {
  ...
  const activeLinks = useMemo(
    () => links.filter(link => new Date(link.expires_at).getTime() > Date.now()),
    [links]
  )
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 22 }}>
        <div>
          ...
          <p style={{ margin: '8px 0 0', fontSize: 13, color: '#6b7280', maxWidth: 620, lineHeight: 1.55 }}>
            Create one-time admin login links for the configured admin account. Each link opens without a password, disappears after a successful use, and the resulting session stays signed in until the user signs out.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(340px, 420px) minmax(0, 1fr)', gap: 18, alignItems: 'start' }}>
        ...
        <section style={panelStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
            <div>
              <h2 style={panelTitleStyle}>Active links</h2>
          ...
          {loading ? (
            ...
            <div style={emptyStateStyle}>No temporary admin links yet.</div>
          ) : (
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          ...
                          <span style={{
                            ...
                            fontSize: 11,
                            fontWeight: 600,
                            background: isExpired ? '#f3f4f6' : '#ecfdf3',
