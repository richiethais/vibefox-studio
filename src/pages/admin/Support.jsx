          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
                {['Client', 'Title', 'Status', 'Date', ''].map(h => (
            ...
            <tbody>
              {rows.map(r => (
                ...
                >
                  <td style={{ padding: '12px 16px', fontWeight: 500, color: '#18181a' }}>{r.clients?.name ?? '—'}</td>
                  <td style={{ padding: '12px 16px', color: '#7a7888' }}>{r.title}</td>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
            <div style={{ background: 'white', borderRadius: '20px 20px 0 0', padding: 24, width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                <div style={{ fontWeight: 600, fontSize: 15, color: '#18181a' }}>{selected.title}</div>
              ...
              <Detail label="Client" value={selected.clients?.name ?? '—'} />
              <Detail label="Email" value={selected.clients?.email ?? '—'} />
              <Detail label="Date" value={new Date(selected.created_at).toLocaleString()} />
              <div style={{ marginTop: 16 }}>
                <div style={{ fontSize: 11, color: '#7a7888', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>Description</div>
                <div style={{ fontSize: 13, color: '#18181a', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{selected.description}</div>
              </div>
              ...
              <button onClick={() => setConfirmDelete(selected)} style={deleteBtnStyle}>Delete ticket</button>
            </div>
          </div>
        )}
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 28, maxWidth: 400, width: '100%', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: '#18181a', marginBottom: 8 }}>Delete support ticket?</div>
