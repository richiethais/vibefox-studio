...
export default function Work() {
  ...
  return (
    <section id="work" ref={ref} style={{ padding: isMobile ? '40px 16px' : '96px 40px' }}>
      <div style={{ maxWidth: 1040, margin: '0 auto' }}>
        <Eyebrow>Demo Projects</Eyebrow>
        <h2 className="fade-up d1" style={{ ...h2Style, fontSize: isMobile ? 'clamp(22px, 6.5vw, 32px)' : h2Style.fontSize, letterSpacing: isMobile ? '-0.5px' : h2Style.letterSpacing }}>What we can <em style={{ fontStyle: 'italic', color: '#b8906a' }}>build.</em></h2>
        ...
        <div className={isMobile ? 'mobile-scroll' : ''} style={isMobile ? {
          ...
          {projects.map((project, i) => (
            <div
              ...
              style={{ background: '#faf9f7', border: '1px solid rgba(0,0,0,0.08)', borderRadius: isMobile ? 12 : 16, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.25s cubic-bezier(0.22,1,0.36,1)', minWidth: isMobile ? '75vw' : undefined, maxWidth: isMobile ? '75vw' : undefined, scrollSnapAlign: isMobile ? 'start' : undefined, flexShrink: isMobile ? 0 : undefined }}
